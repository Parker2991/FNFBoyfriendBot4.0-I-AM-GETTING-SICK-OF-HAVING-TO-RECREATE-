...
function inject (bot, options) {
  bot.console = {
    ...
    useReadlineInterface (rl) {
      ...
      // readline > fix on log
      // messing with console is probably not the best idea but whatever
...
//
  ...
  const lang = require('../lolus.json')
    const ansi = bot.getMessageAsPrismarine(message)?.toAnsi(lang) // It's something in chat/chipmunkmod.js
